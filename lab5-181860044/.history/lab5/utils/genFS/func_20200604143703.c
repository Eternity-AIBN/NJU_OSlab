    ...
    if(destInode->linkCount == 0){  //Need to update the inodeBitmap,blockBitmap,etc.
        ...
        while(destInode->blockCount!=0){
            ...
            if (destInode->blockCount < bound0) {
                destInode->pointer[destInode->blockCount] = blockOffset;
            }
            else if (destInode->blockCount == bound0) {
                getAvailBlock(file, superBlock, &singlyPointerBufferOffset);
                singlyPointerBuffer[0] = blockOffset;
                destInode->singlyPointer = singlyPointerBufferOffset;
                fseek(file, singlyPointerBufferOffset * SECTOR_SIZE, SEEK_SET);
                fwrite((void *)singlyPointerBuffer, sizeof(uint8_t), superBlock->blockSize, file);
            }
                ...
                fread((void *)singlyPointerBuffer, sizeof(uint8_t), superBlock->blockSize, file);
                singlyPointerBuffer[destInode->blockCount - bound0] = blockOffset;
                fseek(file, destInode->singlyPointer * SECTOR_SIZE, SEEK_SET);
                fwrite((void *)singlyPointerBuffer, sizeof(uint8_t), superBlock->blockSize, file);
            }
            else
