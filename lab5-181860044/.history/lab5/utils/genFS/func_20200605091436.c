...
int allocLastBlock (FILE *file, SuperBlock *superBlock, Inode *inode, int inodeOffset, int blockOffset) {
    ...
    if (inode->blockCount < bound0) {
        ...
        printf("inode->singlyPointer=%d\n",inode->singlyPointer);
        fseek(file, inode->singlyPointer * SECTOR_SIZE, SEEK_SET);
        fread((void *)singlyPointerBuffer, sizeof(uint8_t), superBlock->blockSize, file);
        singlyPointerBuffer[inode->blockCount - bound0] = blockOffset;
        printf("singlyPointerBuffer[%d - %d] = %d\n",inode->blockCount,bound0,singlyPointerBuffer[inode->blockCount - bound0]);
    ...
    if(destInode->linkCount == 0){  //Need to update the inodeBitmap,blockBitmap,etc.
        ...
        while(destInode->blockCount!=0){
            ...
            if (destInode->blockCount < bound0) {
                ...
                printf("destInode->singlyPointer=%d\n",destInode->singlyPointer);
                fseek(file, destInode->singlyPointer * SECTOR_SIZE, SEEK_SET);
                fread((void *)singlyPointerBuffer, sizeof(uint8_t), superBlock->blockSize, file);
                printf("singlyPointerBuffer[%d - %d] = %d\n",destInode->blockCount,bound0,singlyPointerBuffer[destInode->blockCount - bound0]);
