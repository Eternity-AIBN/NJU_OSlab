...
int removeAllocedBlock (FILE *file, SuperBlock *superBlock, int *blockOffset) {
    ...
    int32_t tmp=superBlock->blockSize/SECTOR_SIZE*8;
    j = (*blockOffset-superBlock->inodeTable-sizeof(Inode)*tmp)/tmp;
    k = (8*j)%8;
    blockBitmap.byte[j] = blockBitmap.byte[j] | (1 << (8 - k));
...
int removeAllocedInode (FILE *file, SuperBlock *superBlock, int *blockOffset) {
    int j = 0;
    int k = 0;
    int inodeBitmapOffset = 0;
    int inodeTableOffset = 0;
    ...
    j = (*blockOffset-superBlock->inodeTable*SECTOR_SIZE)/sizeof(Inode)/8;
    k = (8*j)%8;
...
int freeInode (FILE *file, SuperBlock *superBlock, Inode *fatherInode, int fatherInodeOffset,
    ...
    printf("    name = %s\n",dirEntry[j].name);
    *destInodeOffset = dirEntry[j].inode;  //To be remove
    fseek(file, *destInodeOffset, SEEK_SET);
    fread((void *)destInode, sizeof(Inode), 1, file);
    printf("blockCount = %d  ",destInode->blockCount);
