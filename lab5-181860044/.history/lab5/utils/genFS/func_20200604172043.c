...
int removeAllocedBlock (FILE *file, SuperBlock *superBlock, int *blockOffset) {
    ...
    int32_t tmp=superBlock->blockSize/SECTOR_SIZE;
    j = (*blockOffset-superBlock->inodeTable-sizeof(Inode)*tmp*8)/(tmp*8);
    k = (*blockOffset-superBlock->inodeTable-sizeof(Inode)*tmp*8)/tmp%8;
    printf("j = %d, k = %d\n",j,k);
