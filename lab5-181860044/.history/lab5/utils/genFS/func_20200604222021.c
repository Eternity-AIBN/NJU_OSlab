...
int getAvailInode (FILE *file, SuperBlock *superBlock, int *inodeOffset) {
    ...
    printf("j=%d, k=%d, inodeBitmap.byte[j]=%d\n",j,k,inodeBitmap.byte[j]);
    inodeBitmap.byte[j] = inodeBitmap.byte[j] | (1 << (7 - k));

    *inodeOffset = inodeTableOffset * SECTOR_SIZE + (j * 8 + k) * sizeof(Inode);
    printf("j=%d, k=%d, inodeOffset=%d, inodeBitmap.byte[j]=%d\n",j,k,*inodeOffset,inodeBitmap.byte[j]);
...
int removeAllocedInode (FILE *file, SuperBlock *superBlock, int *inodeOffset) {
    ...
    for(int i=0;i<1024;++i){
        if(inodeBitmap.byte[i]!=0)
            printf("inodeBitmap.byte[%d]=%d\n",i,inodeBitmap.byte[i]);
    }
