...
void syscallWriteShMem(struct TrapFrame *tf) {
	...
	char character = 0;
	int ttmp;
	asm volatile("movw %%es, %0"::"m"(ttmp));
	asm volatile("movw %0, %%es"::"m"(sel));

	while(realSize < size){
		asm volatile("movw %0, %%es"::"m"(sel));
		for(; cur < SECTORS_PER_BLOCK*SECTOR_SIZE; cur++){
			asm volatile("movb %%es:(%1), %0":"=r"(character):"r"(buffer+cur));
			tmp[realSize] = character;
...
void syscallReadShMem(struct TrapFrame *tf) {
	...
	/* putString("inode.size = ");
	putInt(inode.size);
	putString("file[fd-MAX_DEV_NUM].offset = ");
	putInt(file[fd-MAX_DEV_NUM].offset);*/
