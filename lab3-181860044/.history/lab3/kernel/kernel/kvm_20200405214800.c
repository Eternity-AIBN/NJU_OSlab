...
int loadElf(const char *filename, uint32_t physAddr, uint32_t *entry) {
	...
	struct ELFHeader *elf_hdr = (struct ELFHeader *)elf;

	*entry = elf_hdr->entry;	  // entry address of the program
	int num = elf_hdr->phnum;  //Number of program headers
	int phoff = elf_hdr->phoff;  //Offset of the first program header
	int phsize = ((struct ELFHeader *)elf)->phentsize;  //Size of program headers

	struct ProgramHeader *ph = (struct ProgramHeader *)(elf + phoff); //The first segment
	//physAddr-=ph->vaddr;

	for(uint32_t i=0; i<num; ++i){
		//ph = (struct ProgramHeader *)(elf + phoff + i*phsize);
		if(ph->type==0x1){
			...
			if(ph->memsz > ph->filesz){
				for(int i = 0; i < ph->memsz - ph->filesz; ++i){
					*(uint8_t *)(physAddr+ph->vaddr + ph->filesz + i) = 0;
				}
			}*/
			memcpy((void *)ph->vaddr + physAddr, (void*)elf + ph->off, ph->filesz);
			memset((void *)ph->vaddr + ph->filesz + physAddr, 0, ph->memsz - ph->filesz);
