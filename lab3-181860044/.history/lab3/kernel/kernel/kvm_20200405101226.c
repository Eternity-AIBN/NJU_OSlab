...
int loadElf(const char *filename, uint32_t physAddr, uint32_t *entry) {
	...
	uint8_t buf[200*512];
	fread((void*)buf,sizeof(uint8_t),200*512,file);
	//if(((struct ELFHeader *)buf)->type)
	...
	int phsize = ((struct ELFHeader *)buf)->phentsize;  //Size of program headers
	for(uint32_t i=0; i<num; ++i){
		struct ProgramHeader *ph = (struct ProgramHeader *)(buf + phoff + i*phsize);
		if(ph->type==0x1){
			uint32_t vaddr=ph->vaddr;
			for(uint32_t i=0;i<ph->filesz;++i){
				(void*)(vaddr)+i=(void*)(ph->off)+i;
			}
		}
	}
