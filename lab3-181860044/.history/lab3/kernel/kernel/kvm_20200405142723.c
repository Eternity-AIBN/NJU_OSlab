...
int loadElf(const char *filename, uint32_t physAddr, uint32_t *entry) {
	...
	int ret = readInode(&sBlock, &inode, &inodeOffset, "/usr/print\0");
	if(ret==-1){
		return -1;
	}
	putChar('a');
	...
	*entry = elf_hdr->entry;	  // entry address of the program
	...
	for(uint32_t i=0; i<num; ++i){
		struct ProgramHeader *ph = (struct ProgramHeader *)(elf + phoff + i*phsize);
		if(ph->type==0x1){
			for(int i = 0;i < ph->filesz; ++i){
				*(uint8_t *)(elf + ph->vaddr + i) = *(uint8_t *)(elf + i + ph->off);
			}
			if(ph->memsz > ph->filesz){
				for(int i = 0; i < ph->memsz - ph->filesz; ++i){
					*(uint8_t *)(elf + ph->vaddr + ph->filesz + i) = 0;
				}
			}
		}
	}
