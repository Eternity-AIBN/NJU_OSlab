...
int loadElf(const char *filename, uint32_t physAddr, uint32_t *entry) {
	...
	for (int i = 0; i < 0x100000; i++) {
        *((uint8_t *)0x200000 + i) = *((uint8_t *)0x300000 + i);
    }
	uint8_t *elf = (uint8_t*)0x300000;
	uint8_t buf[16000];
	Inode inode;
	int inodeOffset = 0;
	...
	for(int i=0;i<inode.blockCount*sBlock.blockSize;++i){
		*(buf+i)=*(uint8_t *)(elf+i);
	}
	for (int i = 0; i < 0x100000; i++) {
        *((uint8_t *)0x300000 + i) = *((uint8_t *)0x200000 + i);
    }

	struct ELFHeader *elf_hdr = (struct ELFHeader *)buf;
	...
	*entry = elf_hdr->entry;	  // entry address of the program
	...
	struct ProgramHeader *ph = (struct ProgramHeader *)(buf + phoff); //The first segment

	for(uint32_t i=0; i<num; ++i){
		if(ph->type==0x1){
			for(int i = 0;i < ph->filesz; ++i){
				*(uint8_t *)(physAddr + ph->vaddr + i) = *(uint8_t *)(buf + i + ph->off);
			}
