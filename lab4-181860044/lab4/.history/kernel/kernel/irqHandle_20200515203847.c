...
void syscallWriteShMem(struct TrapFrame *tf) {
	...
	//}
	int tmp = sel|0x11;
	asm volatile("movw %%es, %0"::"m"(sel));
	asm volatile("movw %0, %%es"::"m"(tmp));
	uint8_t ut;
	//asm volatile("movw %0, %%es"::"m"(sel));
	for (i = 0; i < size && index+i < MAX_SHMEM_SIZE; i++) {
		asm volatile("movw %0, %%es"::"m"(tmp));
		asm volatile("movb %%es:(%1), %0":"=r"(ut):"r"(buffer + i));
		asm volatile("movw %0, %%es"::"m"(sel));
	...
	putString("Write: ");
	putInt(shMem[index]);
	putInt(shMem[index+1]);
	putInt(shMem[index+2]);
	putInt(shMem[index+3]);
...
void syscallReadShMem(struct TrapFrame *tf) {
	...
	//}
	asm volatile("movb $0x0, %%es:(%0)"::"r"(buffer));
	asm volatile("movb $0x0, %%es:(%0)"::"r"(buffer+1));
	asm volatile("movb $0x0, %%es:(%0)"::"r"(buffer+2));
	asm volatile("movb $0x0, %%es:(%0)"::"r"(buffer+3));
	pcb[current].regs.eax = i;
	return;
}
