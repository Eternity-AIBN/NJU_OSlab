...
void keyboardHandle(struct TrapFrame *tf) {
	...
	ProcessTable *pt = (ProcessTable*)((uint32_t)(sem[STD_IN].pcb.prev) - (uint32_t)&(((ProcessTable*)0)->blocked));
 	sem[STD_IN].pcb.prev = (sem[STD_IN].pcb.prev)->prev;
 	(sem[STD_IN].pcb.prev)->next = &(sem[STD_IN].pcb);
	return;
}
